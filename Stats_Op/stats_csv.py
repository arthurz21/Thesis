...
# 3. Correlation Matrix Comparison - Modified with larger figure size
def compare_correlation_matrices(df1, df2, region1_name="Region 1", region2_name="Region 2"):
    ...
    # Increased figure size from (18, 6) to (30, 24)
    fig, axes = plt.subplots(1, 3, figsize=(30, 24))

    # Reduced font size for better readability with many features
    sns.set(font_scale=0.7)

    # Create heatmaps with reduced font size for annotations
    sns.heatmap(corr1, ax=axes[0], cmap='coolwarm', vmin=-1, vmax=1,
                xticklabels=1, yticklabels=1)
    axes[0].set_title(f'Correlation Matrix - {region1_name}', fontsize=16)

    sns.heatmap(corr2, ax=axes[1], cmap='coolwarm', vmin=-1, vmax=1,
                xticklabels=1, yticklabels=1)
    axes[1].set_title(f'Correlation Matrix - {region2_name}', fontsize=16)

    sns.heatmap(corr_diff, ax=axes[2], cmap='coolwarm', vmin=-1, vmax=1,
                xticklabels=1, yticklabels=1)
    axes[2].set_title('Correlation Difference (Region1 - Region2)', fontsize=16)

    plt.tight_layout()
    plt.savefig('correlation_comparison.png', dpi=300)  # Increased DPI for better quality
    plt.close()
