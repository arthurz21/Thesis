...
function [results, all_ecg_raw, all_filtered_ecg] = braveheart_batch_my_own(varargin)
disp(['DEBUG: Initial format received = ' varargin{2}]);  % Check what format is passed
...
format = 'DICOM';       % ECG format string
...
folder = varargin{1};     
format = varargin{2};
disp(['DEBUG: Format after input parsing = ' format]);
...
output_ext = varargin{3};  
output_note = varargin{4};  
disp(['Received folder path: ' folder]);
if exist(folder, 'dir')
    disp('Folder exists');
else
...
disp(['DEBUG: Before extension calculation, format = ' format]);
...
disp(['Final source extension to use: ' source_ext]);
% Force DICOM format to use .dcm extension
if strcmpi(format, 'DICOM') || strcmpi(format, 'DICOM.dcm')
    disp('Forcing source extension to .dcm for DICOM files');
    source_ext = '.dcm';
end
...
% Right before the dir() call to list files
disp(['Original file pattern: ' fullfile(file_folder, strcat('*',source_ext))]);

% If we know we're looking for DICOM files but using the wrong extension
if strcmpi(format, 'DICOM') || strcmpi(format, 'DICOM.dcm')
    file_pattern = fullfile(file_folder, '*.dcm');
    disp(['Overriding to pattern: ' file_pattern]);
    file_list_struct = dir(file_pattern);
else
    file_pattern = fullfile(file_folder, strcat('*',source_ext));
    file_list_struct = dir(file_pattern);
end



disp(['Looking for pattern: ' fullfile(file_folder, strcat('*',source_ext))]);
disp(['Found ' num2str(length(file_list_struct)) ' files matching this pattern']);
...
if ~isempty(file_list_struct) && isfield(file_list_struct, 'folder') && ~isempty(file_list_struct(1).folder)
    orig_directory = file_list_struct(1).folder;
elseif ~isempty(file_list_struct) && isfield(file_list_struct, 'path') && ~isempty(file_list_struct(1).path)
    orig_directory = file_list_struct(1).path;
else
    orig_directory = file_folder;  % Fall back to the originally specified folder
end
...
all_ecg_raw = cell(num_files,1);
all_filtered_ecg = cell(num_files,1);
...
parfor (i = 1:num_files, workers)
	...
	try   % To deal with if ECG wont load due to some error in formatting
        ...
        disp(['Loaded ECG with format: ' format ' and file: ' char(file_list{i})]);
        ...
        all_ecg_raw{i} = ecg_raw;
        all_filtered_ecg{i} = filtered_ecg;
