...
ecg_folder = "C:\Users\arthu\PycharmProjects\Thesis\NYGH";
...
dicom_files = dir(fullfile(ecg_folder, '*.dcm'));
disp(['Found ' num2str(length(dicom_files)) ' DICOM files']);
...
for patient_idx = 1:num_patients
    ...
    feature_value{1} = strcat('patient_', num2str(patient_idx));
    ...
    feature_value{28} = str2double(a{patient_idx, 366+1});
    feature_value{29} = str2double(a{patient_idx, 368+1});
    feature_value{30} = str2double(a{patient_idx, 370+1});
    feature_value{31} = str2double(a{patient_idx, 352+1}); 
    ...
    feature_value{41} = str2double(a{patient_idx, 400});
